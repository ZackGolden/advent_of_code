...
def find_region(x, y, map):
    def go(x, y, region):
        ...
        for dir in dirs:
            if (
                (dir in map)
                and (dir not in region)
                and (map[(dir[0], dir[1])] == plant)
            ):
...
def part_1(lines):
    ...
    while len(map) > 0:
        ...
        regions.append((map[current], find_region(current[0], current[1], map)))
    ...
    for x, y in region[1]:
        ...
        for i in range(8):
            if i % 2 == 0:
                ...
                if (
                    (dirs[i] in region[1])
                    and (dirs[(i + 2) % 8] in region[1])
                    and (dirs[(i + 1) % 8] not in region[1])
                ):
                    corners += 1
    sides = corners
    return sides
