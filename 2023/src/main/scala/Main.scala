...
@main def hello: Unit =
  ...
  println("Part 1: " + generateCalibrationValue(Source.fromFile(filename).getLines.toList)) // 54940
  println("Part 2: " + generateCalibrationValue(Source.fromFile(filename).getLines.toList.map(wordToNumber(_)))) // 54208
